...
foreach ( $postArray as $sForm => $value )
{
	...
	if(!empty($edit_beitrag))
	{
		...
		$content_sql = mysql_real_escape_string($_POST["editor"], $dbconnection);
		
		// echo 'Beitrag SQL'.$beitrag_sql;
		
		$query = "
			UPDATE tblBeitraege
				SET Beitrag = '$content_sql'
				WHERE BeitragID = '$edit_beitrag'
		";
		echo $query;
	...
	if(empty($edit_beitrag) and !empty($_POST["editor"]))
	{
		$dbconnection = mysql_connect($dbhost, $dbuser, $dbpass) or die(mysql_error());
		mysql_select_db($dbname,$dbconnection) or die(mysql_error());
